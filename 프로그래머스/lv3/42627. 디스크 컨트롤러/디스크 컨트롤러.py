...
def solution(jobs):
    ...
    while i < len(jobs):
        for job in jobs:
            if start < job[0] <= now: # 작업 요청 시점이 이전에 끝낸 작업보다 크고 현재 시간보다 작을 경우 작업 가능
                heapq.heappush(heap, [job[1], job[0]]) # (처리 시간, 요청 시간) 형태로 힙에 저장
        
        if len(heap) > 0: # 힙에 처리할 작업이 남은 경우
            current = heapq.heappop(heap) #cur은 현재 처리중인 작업
            start = now
            now += current[0]
            answer += (now - current[1]) # 작업 요청시간으로부터 처리 완료까지의 시간을 계산하여 answer에 저장
            i += 1
